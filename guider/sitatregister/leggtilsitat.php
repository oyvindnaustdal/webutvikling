...
if (!$datasett) {
    ...
    exit; // htmlspecialchars() gjer at eventuell HTML blir vist som tekst, ikkje køyrd som kode.
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["submit"])) {
    ...
    // trim() fjernar mellomrom først og sist i teksten, slik at ein ikkje får unødvendige blankteikn.
    $tekst = trim($_POST["tekst"] ?? ""); 
    
    //FILTER_VALIDATE_INT gjer at PHP sjekkar om verdien faktisk er eit heiltal.
    $personid = filter_input(INPUT_POST, "lstPerson", FILTER_VALIDATE_INT); 

    // Fjernar eventuell HTML for tryggleik (om du ikkje ønskjer HTML i tekstfeltet)
    $tekst = strip_tags($tekst);

    // Sjekkar at begge felt faktisk har verdiar
    if (empty($tekst) || !$personid) {
